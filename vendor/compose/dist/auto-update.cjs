...
// src/auto-update/index.ts
var auto_update_exports = {};
__export(auto_update_exports, {
  DEFAULT_CAPTURE_SETTINGS: () => DEFAULT_CAPTURE_SETTINGS,
  NOCHANGE_MAX_PCT: () => NOCHANGE_MAX_PCT,
  STRUCTURE_STABLE_MIN: () => STRUCTURE_STABLE_MIN,
  captureUrl: () => captureUrl,
  checkCleanData: () => checkCleanData,
  classify: () => classify,
  ...
  isStructureStable: () => isStructureStable,
...
// src/auto-update/signature.ts
var STRUCTURE_STABLE_MIN = 0.8;
function structureSignature(classification) {
  const s = classification.signals;
  const shapes = uniqueSorted(s.repeated.map((g) => `${g.kind}@${g.selector}`));
  const anchors = uniqueSorted(s.sampleStableAnchors);
  return { shapes, anchors, formFields: s.formFields, eligible: classification.eligible };
}
function signatureSimilarity(a, b) {
  const setA = /* @__PURE__ */ new Set([...a.shapes, ...a.anchors]);
  const setB = /* @__PURE__ */ new Set([...b.shapes, ...b.anchors]);
...
function isStructureStable(prev, next) {
  if (!next.eligible) return false;
  if (prev.eligible !== next.eligible) return false;
  if (prev.formFields !== next.formFields) return false;
  return signatureSimilarity(prev, next) >= STRUCTURE_STABLE_MIN;
}
function uniqueSorted(values) {
  return Array.from(new Set(values)).sort();
}

// src/auto-update/decide.ts
var NOCHANGE_MAX_PCT = 0.05;
function isUnchanged(diff) {
  return diff.pixelDiffPct <= NOCHANGE_MAX_PCT;
}
function decideUpdate(input) {
  const { prevSignature, evaluation, changed } = input;
  const signature = structureSignature(evaluation.classification);
  const eligible = evaluation.classification.eligible;
  const qualityPass = evaluation.quality?.pass ?? false;
  const metrics = evaluation.quality ? { pixelDiffPct: evaluation.quality.pixelDiffPct, ssim: evaluation.quality.ssim } : null;
  const structureStable = prevSignature ? isStructureStable(prevSignature, signature) : eligible;
  if (!changed) {
    return {
      action: "skip",
      reason: "recapture is reference-identical (no product drift) \u2014 nothing to publish",
      signature,
      changed,
      eligible,
      qualityPass,
  ...
  if (!eligible) {
    return verdict2("flag", `screen lost reconstruction eligibility: ${reasonsOf(evaluation)}`, base());
  }
  if (!structureStable) {
    return verdict2("flag", "structural redesign: anchors/shape diverged from the prior accepted render", base());
  }
  if (!qualityPass) {
    return verdict2(
      "flag",
      `reconstruction fails the calibrated quality gate${metrics ? ` (pixelDiff=${metrics.pixelDiffPct}% ssim=${metrics.ssim})` : ""}`,
      base()
    );
  }
  return verdict2(
    "publish",
    `data changed; structure-stable, eligible, reconstruction within the calibrated bar${metrics ? ` (pixelDiff=${metrics.pixelDiffPct}% ssim=${metrics.ssim})` : ""}`,
    base()
  );
  function base() {
    return { signature, changed, eligible, qualityPass, structureStable, metrics };
  }
  function verdict2(action, reason, rest) {
    return { action, reason, ...rest };
  }
}
function reasonsOf(evaluation) {
  const ineligible = evaluation.classification.reasons.filter(
    (r) => r.includes("ineligible") || r.includes("fail-safe")
  );
  return (ineligible.length ? ineligible : evaluation.classification.reasons).join("; ");
}
...
function serializeHybridInPage() {
      ...
      fontFaceTasks.push(
        (async () => {
          ...
          try {
            ...
            const dataUri2 = await new Promise((res) => {
              const fr = new FileReader();
              fr.onload = () => res(fr.result);
              fr.onerror = () => res(null);
  ...
  function clone(el) {
    ...
    if (kind) {
      ...
      let dataUri2 = null;
      if (kind === "canvas") dataUri2 = tryCanvasDataUri(el);
      else if (kind === "video") dataUri2 = tryVideoDataUri(el);
...
async function loadPng(path) {
  return toRgba(import_pngjs.PNG.sync.read(await (0, import_promises3.readFile)(path)));
}
...
async function writeProof(opts) {
  await (0, import_promises4.mkdir)(opts.outDir, { recursive: true });
  const htmlPath = (0, import_node_path4.join)(opts.outDir, "index.html");
  await (0, import_promises4.writeFile)(htmlPath, renderProofHtml(opts), "utf8");
  return htmlPath;
}
